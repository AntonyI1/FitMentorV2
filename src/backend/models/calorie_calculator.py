...
def validate_input(data):
    ...
    if not isinstance(weight, (int, float)) or weight < 40 or weight > 200:
        raise ValidationError("weight must be between 40 and 200 kg")
...
def calculate_macros(target_calories, weight_kg):
    """Calculate macronutrient breakdown."""
    weight_lbs = weight_kg * 2.205

    protein_grams = round(weight_lbs * 1.0)
    ...
    total = protein_calories + carb_calories + fat_calories
    ...
    return {
        ...
        "carbs": {
            ...
            "percentage": round(carb_calories / total * 100) if total > 0 else 0,
        },
        "fats": {
            "grams": fat_grams,
...
def calculate(data):
    ...
    macros = calculate_macros(target, data["weight"])
